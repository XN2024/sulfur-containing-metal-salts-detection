...
class TextCNN(nn.Module):
    def __init__(self, inputsize,embedding_dim,hidden_dim,out_size,batch_size=1,n_layer = 1, dropout = 0,
                 embedding = None):
        super(TextCNN, self).__init__()    #   这里的5是指输入的特征数
        ...
        #                   num_layers=self.n_layer, bidirectional=False)
        self.cnn = nn.Conv1d(in_channels=1, out_channels=1,kernel_size=3,stride=1)
        self.second=nn.Sequential(nn.ReLU(),
                                  nn.MaxPool1d(2,1)# Loan_application_Configuration4
                                        # nn.ReLU(),
                                        # nn.Linear(45, 30),
                                        # nn.ReLU(),
                                        # nn.Linear(30, 5),
    ...
    def forward(self, x):
        ...
        output0 = self.cnn(input).squeeze()
        hn0=self.second(output0)
        # print(0)
