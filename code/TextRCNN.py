...
class TextRCNN(nn.Module):
    def __init__(self, inputsize,embedding_dim,hidden_dim,out_size,batch_size=1,n_layer = 1, dropout = 0,
                 embedding = None):
        super(TextRCNN, self).__init__()    #   这里的5是指输入的特征数
    ...
    def forward(self, x):
        ...
        # print(0)
        # s = torch.load("myfinalHideState5weiLSTMhelpdesk1.pth")
        # q = final_hidden_state[-1].detach().numpy().reshape(9, 10)
        # temp = np.vstack((s, q))
        # #
        # # torch.save(q, "myfinalHideState5weiLSTMhelpdesk1.pth")
        # torch.save(temp, "myfinalHideState5weiLSTMhelpdesk1.pth")
        out = torch.cat((input, output0), 2)
        out=F.relu(out)
