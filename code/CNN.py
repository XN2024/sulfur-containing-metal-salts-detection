...
class CNN(nn.Module):
    def __init__(self, inputsize,embedding_dim,hidden_dim,out_size,batch_size=1,n_layer = 1, dropout = 0,
                 embedding = None):
        super(CNN, self).__init__()    #   这里的5是指输入的特征数
        ...
        #                   num_layers=self.n_layer, bidirectional=False)
        self.cnn = nn.Conv1d(in_channels=5, out_channels=1,kernel_size=1)
    ...
    def forward(self, x):
        first = self.firstLayer(x).to(torch.float32)
