...
numclass=5
df = pd.read_excel('添加干扰物质后UV+FL选择性指纹图.xlsx')
savefilename='final_fig/添加干扰物质后UV+FL选择性指纹图_0.png'
UV_weight=0
...
for i in range(len(z_1)):
    ...
    if label[i]==labelclass[5]:
        type5_x.append(final_NEW[:, 0][i])
        type5_y.append(final_NEW[:, 1][i])
        type5_z.append(final_NEW[:, 2][i])

    if label[i]==labelclass[6]:
        type6_x.append(final_NEW[:, 0][i])
        type6_y.append(final_NEW[:, 1][i])
        type6_z.append(final_NEW[:, 2][i])

    if label[i]==labelclass[7]:
        type7_x.append(final_NEW[:, 0][i])
        type7_y.append(final_NEW[:, 1][i])
        type7_z.append(final_NEW[:, 2][i])

    if label[i]==labelclass[8]:
        type8_x.append(final_NEW[:, 0][i])
        type8_y.append(final_NEW[:, 1][i])
        type8_z.append(final_NEW[:, 2][i])

    if label[i]==labelclass[9]:
        type9_x.append(final_NEW[:, 0][i])
        type9_y.append(final_NEW[:, 1][i])
        type9_z.append(final_NEW[:, 2][i])
    if label[i]==labelclass[10]:
        type10_x.append(final_NEW[:, 0][i])
        type10_y.append(final_NEW[:, 1][i])
        type10_z.append(final_NEW[:, 2][i])

    if label[i]==labelclass[11]:
        type11_x.append(final_NEW[:, 0][i])
        type11_y.append(final_NEW[:, 1][i])
        type11_z.append(final_NEW[:, 2][i])
...
ax5=ax.scatter3D(type5_x, type5_y, type5_z,c=color1[5],marker ='o',s=50)
ax6=ax.scatter3D(type6_x, type6_y, type6_z,c=color1[6],marker ='o',s=50)
ax7=ax.scatter3D(type7_x, type7_y, type7_z,c=color1[7],marker ='o',s=50)
ax8=ax.scatter3D(type8_x, type8_y, type8_z,c=color1[8],marker ='o',s=50)
ax9=ax.scatter3D(type9_x, type9_y, type9_z,c=color1[9],marker ='o',s=50)
ax10=ax.scatter3D(type10_x, type10_y, type10_z,c=color1[10],marker ='o',s=50)
ax11=ax.scatter3D(type11_x, type11_y, type11_z,c=color1[11],marker ='o',s=50)
...
handles = [ax0,ax1, ax2, ax3, ax4,ax5,ax6, ax7, ax8, ax9,ax10, ax11]
ax.set_title(title, fontsize=20)
ax.legend(handles=handles,labels=labelclass, mode="expand", ncol = 6, borderaxespad = 0)
